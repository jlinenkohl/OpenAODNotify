...
### Notification-Triggered-Overlay Count Telemetry
- **Decision**: Added a second, parallel 7-slot ring-buffer tally counting how many *valid* notifications actually triggered the overlay/breathing effect per day, surfaced alongside the overlay-visible-time telemetry.
- **Reasoning**: Overlay-visible time alone conflates two very different things — genuine AOD/ambient-display being on for unrelated reasons (e.g. overnight while charging) vs. this app actively forcing the overlay on in response to a notification. Real-world telemetry showed ~10hrs/day of overlay-visible time, which is plausible for general AOD but not for notification-driven activity alone. Since the app cannot directly distinguish "native AOD is on" from "our overlay forced it on" using public APIs, notification count is used as an inferential proxy: a day with low overlay-visible time but many notifications suggests efficient short-lived breathing sessions, while high overlay time with few notifications suggests the time is mostly attributable to ambient display being on for other reasons.
- **Implementation**: Identical fixed-size ring-buffer pattern as the overlay-visible-time telemetry (`NOTIF_SLOT_COUNT = 7`, keyed by `epochDay % 7`, 14 fixed `SharedPreferences` keys forever, no pruning). Incremented once per valid notification in `OpenAODListener.onNotificationPosted()` (`PreferenceUtils.incrementNotificationCountForDate()`), immediately after `isValidNotification()` passes — i.e. only counts notifications that actually pass this app's own filtering rules, not every notification the OS delivers. Rendered as a second column next to the existing overlay-visible-time table in the debug panel.
- **Pitfall**: This counts notification *arrivals* that pass filtering, not actual breathing-animation duration per notification — it's a proxy, not a precise measurement. A burst of notifications in quick succession while the overlay is already visible will still increment the counter once per notification even though it may not meaningfully extend visible time.

### Explicit Release-Signing Verification Task
- **Decision**: Added a custom `verifyReleaseSigning` Gradle task (`app/build.gradle.kts`) rather than relying solely on the implicit signing baked into `packageRelease`.
- **Reasoning**: AGP signs the release APK silently as part of `assembleRelease` — there's no dedicated, visible task for it, and no built-in check that the *correct* certificate was actually used (a missing/stale `local.properties`, wrong keystore path, or empty password could silently produce an unsigned or wrong-cert APK without an obvious build failure, since `signingConfigs.findByName("release")?.let {...}` intentionally no-ops when credentials are absent to keep IDE Run builds working).
- **Implementation**: `verifyReleaseSigning` depends on `assembleRelease`, then shells out to `apksigner verify --print-certs` (locating the Android SDK build-tools via `local.properties`' `sdk.dir` or `ANDROID_HOME`/`ANDROID_SDK_ROOT`) and asserts the resulting certificate's SHA-256 digest matches a hardcoded `expectedReleaseCertSha256` constant. Fails the build loudly (non-zero exit, clear `GradleException` message) if the APK is unsigned, or signed with an unexpected certificate.
- **Release Protocol integration**: Run as `./gradlew clean assembleRelease verifyReleaseSigning` (see `.github/copilot-instructions.md` Release Protocol step 5) — must pass before tagging/pushing/publishing a release.
- **Pitfall**: If the release signing certificate is ever intentionally rotated, `expectedReleaseCertSha256` in `app/build.gradle.kts` must be updated to the new certificate's SHA-256 digest, or every future release build will fail this check.
